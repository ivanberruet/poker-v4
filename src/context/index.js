...
export function AppWrapper({ children }) {
  ...
  const [settings, setSettings] = useState({
    ...
    money: {
      entry: 5000,
      reentry: 5000,
      prizes: { pool: 0, winners: 2, percentages: [70, 30, 15] }
    },
    time: { timePerLevel: 15, initialLevel: 1 },
    chips: { available: [], distribution: [] },
    structure: {},
    isLoading: true,
  });
    ...
    const fetchData = async () => {
      ...
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/chips`),
        ...
        setSettings(oldSettings => ({
          ...oldSettings,
          players: { ...oldSettings.players, dbPlayers: playersData, availablePlayers: playersData },
          chips: {
            ...oldSettings.chips,
            available: chipsData.map(chip => ({
              ...chip,
              active: true,
  ...
  useEffect(() => {
    if (game.endTime) {
      toast({
        ...
        description: `Felicitaciones ${settings.players?.registeredPlayers?.activePlayers?.[0]?.nick || "Jugador"}!`,
        variant: "outline",
      });
  ...
  return (
    <AppContext.Provider value={{ settings, setSettings, history, game, setGame }}>
      <Confetti run={Boolean(game.endTime)} recycle={false} numberOfPieces={10000} tweenDuration={4000} width={width} height={height} friction={0.99} />
      {children}
    </AppContext.Provider>
  );
}
